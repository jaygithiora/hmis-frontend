...
import { SchemePreauthsService } from '@services/dashboard/masters/insurances/scheme-preauths/scheme-preauths.service';
...
@Component({
  selector: 'app-scheme-preauths',
  templateUrl: './scheme-preauths.component.html',
  styleUrl: './scheme-preauths.component.scss'
})
export class SchemePreauthsComponent implements OnInit {
  ...
  loadPage(page: number): void {
    this.isLoading = true;
    this.schemePreauthsService.getSchemePreauths(page).subscribe((result: any) => {
      console.log(result);
      this.isLoading = false;
      this.scheme_preauths = result.scheme_preauths.data;// Set the items
      this.totalItems = result.scheme_preauths.total; // Total number of items
      this.perPage = result.scheme_preauths.per_page; // Items per page
      this.currentPage = result.scheme_preauths.current_page; // Set the current page
      this.toItems = result.scheme_preauths.to; // Set to Items
      this.fromItems = result.scheme_preauths.from; // Set from Items
    }, error => {
      this.isLoading = false;
      console.log(error);
    });
  ...
  addSchemePreauth() {
    if (this.schemePreauthForm.valid) {
      ...
      this.isLoading = true;
      this.schemePreauthsService.updateSchemePreauth(this.schemePreauthForm.getRawValue()).subscribe((result: any) => {
        this.isLoading = false;
        if (result.success) {
