...
export class DoctorDepartmentsComponent implements OnInit {
    ...
    constructor(
        ...
        private doctorDepartmentsService: DoctorDepartmentsService,
    ...
    loadPage(page: number) {
        this.isLoading = true;
        this.doctorDepartmentsService.getDoctorDepartments(page).subscribe(
            (result: any) => {
        ...
        if (doctorDepartment != null) {
            ...
            if (doctorDepartment.doctor_id != null) {
                this.doctors.push({
                    ...
                    name: `${doctorDepartment.doctor?.firstname} ${doctorDepartment.doctor?.other_names} (${doctorDepartment.doctor?.code})`
                });
    ...
    addDoctorDepartment() {
        if (this.doctorDepartmentForm.valid) {
            this.isLoading = true;
            this.doctorDepartmentsService
                .updateDoctorDepartment(this.doctorDepartmentForm.getRawValue())
                .subscribe(
                    (result: any) => {
                        this.isLoading = false;
                        if (result.success) {
                    ...
                    (error) => {
                        ...
                        if (error?.error?.errors?.specialization) {
                            this.toastr.error(
                                error?.error?.errors?.specialization
                            );
                        }
