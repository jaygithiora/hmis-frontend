...
export class PatientRegistrationComponent implements OnInit, AfterViewInit {
    ...
    loadingIdentificationDocumentTypes: boolean = false;
    ...
    identification_document_types: any[] = [];
    genders = [
        {id: 'MALE', name: 'MALE'},
    ...
    searchIdentificationDocumentTypes$ = new Subject<string>();
    ...
    selectedIdentificationDocumentTypeOption: any;
    ...
    constructor(
        ...
        private bloodGroupService: BloodGroupsService,
        private nextOfKinRelationService: NextOfKinRelationsService,
        private identificationDocumentTypesService: IdentificationDocumentTypesService,
        ...
        this.patientRegistrationForm = this.fb.group({
            ...
            identification_document_type: ['', [Validators.required]],
            phone: [
                '',
                [
                    Validators.required,
    ...
    setupSearch() {
        this.search$
            .pipe(
                ...
                switchMap((term) =>
                    this.organizationService.getOrganizations(1, term)
        ...
        this.searchSalutation$
            .pipe(
                ...
                switchMap((term) =>
                    this.salutationService.getSalutations(1, term)
                ) // Switch to a new observable for each search term
            )
            .subscribe((results: any) => {
        ...
        this.searchBloodGroup$
            .pipe(
                ...
                switchMap((term) =>
                    this.bloodGroupService.getBloodGroups(1, term)
                ) // Switch to a new observable for each search term
            )
            .subscribe((results: any) => {
        ...
        this.searchNextOfKinRelation$
            .pipe(
                ...
                switchMap((term) =>
                    this.nextOfKinRelationService.getNextOfKinRelations(1, term)
                ) // Switch to a new observable for each search term
            )
            .subscribe((results: any) => {
        ...
        this.searchInsurances$
            .pipe(
                ...
                switchMap((term) =>
                    this.insuranceService.getInsurances(1, term)
                ) // Switch to a new observable for each search term
            )
            .subscribe((results: any) => {
        ...
        this.searchSchemes$
            .pipe(
                ...
                switchMap(({term, insuranceId}) =>
                    this.schemesService.getSchemes(1, term, insuranceId)
                ) // Switch to a new observable for each search term
            )
            .subscribe((results: any) => {
        ...
        this.searchIdentificationDocumentTypes$
            .pipe(
                debounceTime(300), // Wait for the user to stop typing for 300ms
                distinctUntilChanged(), // Only search if the query has changed
                tap(() => (this.loadingIdentificationDocumentTypes = true)), // Show the loading spinner
                switchMap((term) =>
                    this.identificationDocumentTypesService.getIdentificationDocumentTypes(
                        1,
                        term
                    )
                ) // Switch to a new observable for each search term
            ...
            .subscribe((results: any) => {
                this.identification_document_types =
                    results.identification_document_types.data;
                this.loadingIdentificationDocumentTypes = false; // Hide the loading spinner when the API call finishes
            });
    }
    ...
    addScheme() {
        if (this.patient_schemes.length > 0) {
            if (this.patient_schemes.invalid) {
                this.toastr.error(
                    'Please fill all allergy details before adding a new one.'
                );
                return;
            }
        }
    ...
    onPatientSelect(event: any) {
        ...
        if (this.patient.identification_document_type) {
            this.identification_document_types = [];
            this.identification_document_types.push(
                this.patient.identification_document_type
            );
            this.selectedIdentificationDocumentTypeOption =
                this.patient.identification_document_type_id;
        }
        this.patientRegistrationForm
            .get('gender')
            ?.setValue(this.patient.gender);
        this.patientRegistrationForm.get('dob')?.setValue(this.patient.dob);
        this.patientRegistrationForm
            .get('first_name')
            ?.setValue(this.patient.first_name);
        this.patientRegistrationForm
            .get('other_names')
            ?.setValue(this.patient.other_names);
        this.patientRegistrationForm
            .get('id_number')
            ?.setValue(this.patient.id_number);
        this.patientRegistrationForm
            .get('member_number')
            ?.setValue(this.patient.member_number);
        this.patientRegistrationForm
            .get('id_number')
            ?.setValue(this.patient.id_number);
        this.patientRegistrationForm
            .get('member_type')
            ?.setValue(this.patient.member_type);
        this.patientRegistrationForm
            .get('guardian_name')
            ?.setValue(this.patient.guardian_name);
        ...
        this.patientRegistrationForm
            .get('patient_location')
            ?.setValue(this.patient.patient_location);
        this.patientRegistrationForm
            .get('longitude')
            ?.setValue(this.patient.longitude);
        this.patientRegistrationForm
            .get('latitude')
            ?.setValue(this.patient.latitude);
        this.patientRegistrationForm
            .get('citizenship')
            ?.setValue(this.patient.citizenship);
        ...
        this.patientRegistrationForm
            .get('next_of_kin_name')
            ?.setValue(this.patient.next_of_kin_name);
        this.patientRegistrationForm
            .get('next_of_kin_phone')
            ?.setValue(this.patient.next_of_kin_phone);
        ...
        if (this.patient.next_of_kin_relation) {
            ...
            this.selectedNextOfKinRelationOption =
                this.patient.next_of_kin_relation_id;
        }
        this.patient.patient_schemes.forEach((scheme) => {
            const patientSchemeGroup = this.fb.group({
                id: [scheme.id || '', []],
                insurance: [
                    scheme.scheme.insurance_id || null,
                    Validators.required
                ],
    ...
    onInsuranceSelect(event: any, index: number) {
        ...
        this.searchSchemes$.next({
            term: '',
            insuranceId // empty = load all schemes for that insurance
        });
    }
    onSchemeSearch(term: string, index: number) {
        const insuranceId = this.patient_schemes
            .at(index)
            .get('insurance')?.value;
    ...
    ngOnInit() {
        ...
        if (id != null) {
            ...
            this.patientRegistrationService
                .getPatientRegistration(parseInt(id))
                .subscribe(
                    (result: any) => {
        ...
        if (this.patientRegistrationForm.valid) {
            ...
            if (this.patientImage != null) {
                ...
                const blob = this.dataURLtoBlob(
                    this.patientImage.imageAsDataUrl
                );
            ...
            this.patientRegistrationService
                .updatePatientRegistration(
                    this.patientImage != null
                        ? formData
                        : this.patientRegistrationForm.getRawValue()
                )
                .subscribe(
                    (result: any) => {
                        if (result.success) {
                    ...
                    (error) => {
                        ...
                        if (
                            error?.error?.errors?.identification_type_document
                        ) {
                            this.toastr.error(
                                error?.error?.identification_type_document
                            );
                        }
                        ...
                        if (error?.error?.errors?.next_of_kin_relation) {
                            this.toastr.error(
                                error?.error?.next_of_kin_relation
                            );
                        }
