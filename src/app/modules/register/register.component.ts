...
export class RegisterComponent implements OnInit, OnDestroy {
    ...
    loadSubscriptionPackages(page: number) {
        this.isLoading = true;
        this.indexService.getSubscriptionPackages(page).subscribe(
            (result: any) => {
                console.log(result);
    ...
    initOrganizationStep() {
        setTimeout(() => {
            if (!this.searchLocation /*|| this.autocompleteInitialized*/)
                return;
    ...
    async register() {
        if (this.isAllFormsValid()) {
            ...
            Object.entries(registrationData).forEach(([key, value]: any) => {
            ...
            this.authService.register(formData).subscribe(
                (result: any) => {
                    if (result.access_token && result.user) {
                        ...
                        this.toastr.error(
                            'Access token could not be found! Try again!'
                        );
                    }
                ...
                (error: any) => {
                    ...
                    if (error?.error?.errors) {
                        ...
                        if (error?.error?.errors?.package_id) {
                            this.toastr.error(error?.error?.errors?.package_id);
                        }
                        ...
                        if (error?.error?.errors?.organization_email) {
                            this.toastr.error(
                                error?.error?.errors?.organization_email
                            );
                        }
                        if (error?.error?.errors?.organization_phone) {
                            this.toastr.error(
                                error?.error?.errors?.organization_phone
                            );
                        }
    ...
    isAllFormsValid(): boolean {
        return this.registerForm.valid && this.organizationForm.valid;
    }
